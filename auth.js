...
function renderPinUI(onSuccess) {
  // オーバーレイがbodyより前面に出るのでbody自体は隠さない
  ...
  style.textContent = `
    ...
    .pin-box { text-align: center; padding: 0 1.5rem; }
    .pin-logo {
    ...
    .pin-dots span.shake  { background: #E05555; border-color: #E05555; }
    .pin-grid {
    ...
    .pin-key:active { background: rgba(212,136,58,.35); transform: scale(.92); }
  ...
  function updateDots(state) {
    document.querySelectorAll('.pin-dots span').forEach((s, i) => {
      s.className = '';
      if (state === 'error') s.classList.add('shake');
      else if (i < entered.length) s.classList.add('filled');
    });
  }
